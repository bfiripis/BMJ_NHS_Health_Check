#' Calculate Aggregate Annual Outcomes for Microsimulation
#' 
...
#' @return Named list with aggregate outcomes for the year
...
calculate_aggregate_outcomes <- function(population, 
  ...
  # Calculate life years (LYs) - living individuals contribute 1 LY each
  # Dead individuals contribute 0.5 LY (assuming mid-year death)
  life_years <- sum(population$alive, na.rm = TRUE) + 
    (dead_this_year * 0.5)
  ...
  # Calculate disease prevalence (proportion with disease among living)
  living_pop <- population %>% filter(alive == TRUE)
  ...
  # Calculate disease incidence (new cases this year)
  ...
  for (disease in diseases) {
    ...
    incidence_results[[paste0(disease, "_incidence_rate")]] <- 
      ifelse(alive_population > 0, new_cases / alive_population * 1000, 0)  # per 1000 person-years
  }
  
  # Calculate deaths by cause
  deaths_by_cause <- list()
  if ("cause_of_death" %in% names(population)) {
    death_causes <- population %>% 
      filter(!is.na(death_year), death_year == current_year) %>%
      filter(!is.na(cause_of_death), cause_of_death != "") %>%
      filter(cause_of_death != "NA") %>%  # Handle string "NA"
      count(cause_of_death, .drop = FALSE) %>%  # .drop = FALSE ensures no NA groups
      rename(count = n) %>%
      filter(!is.na(cause_of_death))  # Final safety check
    ...
    if (nrow(death_causes) > 0) {
      for (i in 1:nrow(death_causes)) {
        ...
        # Only create column if cause is not empty/NA
  ...
  # Add deaths by cause
  outcomes <- c(outcomes, deaths_by_cause)
...
calculate_individual_utilities <- function(population,
  ...
  pop_with_utilities <- population %>%
    mutate(
      utility_age = pmin(age, max(age_sex_utilities$Age, na.rm = TRUE))
    )
  
  # Merge baseline utilities
  ...
  if (method == "minimum") {
    ...
    for (disease in diseases) {
      ...
      disease_utils <- disease_utilities %>%
        filter(tolower(Disease) == disease_clean) %>%
        dplyr::select(sex_label, Utility) %>%
...
#' Initialize Model Outputs Dataframe
#'
#' Create empty model_outputs dataframe with proper column structure
#'
#' @param diseases Vector of disease names
...
initialize_model_outputs <- function(diseases = c("chd", "stroke", "copd", "lung_cancer", "colorectal_cancer")) {
  ...
  # Disease prevalence columns
  prevalence_cols <- paste0(diseases, "_prevalence")
  
  # Disease incidence columns  
  ...
  # Combine all columns
  all_cols <- c(base_cols, disease_cost_cols, prevalence_cols, incidence_cols, death_cols)
